...
export default function ChatBox({ currentPage }) {
  ...
  const getPageSpecificAdvice = () => {
    switch (currentPage) {
      case "landing":
        return "You're on the landing page. If you want to view jobs, click on the suitcase icon. If you're looking to connect with mentors, click on the two users icon.";
      case "jobs":
        return "You're on the job page. If you want to search for other users, click on the magnifying glass icon. If you want to go back to the home page, click on the exit icon.";
      case "groups":
        return "You're on the groups page. Use this page to interact with your groups or chat with mentors and mentees. To look for jobs, click on the suitcase icon.";
      default:
        return "Navigate through the site using the icons on the navigation bar to search for users, view jobs, interact in groups, or log out.";
    }
  };
  const model_config = `
    ...
    Just to clarify you are an AI assistant, created to help users navigate a career advice website and offer general guidance. 
...
*/
  ...
  return (
    <div className="fixed bottom-0 left-0 z-30">
      <button
        className="fixed bottom-8 left-9 outline-none border-none h-12 w-12 flex cursor-pointer items-center justify-center rounded-full bg-primary-light text-white transition-transform duration-200 z-10100"
        onClick={toggleChat}
      >
        <FontAwesomeIcon icon={isChatOpen ? faClose : faCommentDots} />
      </button>
      <div
        className={`fixed left-9 bottom-[5.625rem] w-[26.25rem] bg-white rounded-2xl overflow-hidden transform transition-all duration-100 ${
          isChatOpen
      ...
      >
        ...
        <ul className="overflow-y-auto h-[31.875rem] p-8 pb-20">
          {messages.map((msg, index) => (
            ...
            >
              <p
                className={`whitespace-pre-wrap px-4 py-3 rounded-lg max-w-[75%] text-sm ${
                  msg.role === "assistant"
                    ? "text-black bg-gray-200 rounded-l-none"
                    : "text-white bg-blue-500 rounded-r-none"
                }`}
              >
                {msg.content}
              </p>
        ...
        <div className="flex gap-1 absolute bottom-0 w-full bg-white p-1 px-5 border-t border-gray-200">
          ...
          <FontAwesomeIcon
            icon={faPaperPlane}
            className="self-end text-primary cursor-pointer flex items-center text-2xl"
            onClick={sendMessage}
          />
        </div>
      </div>
