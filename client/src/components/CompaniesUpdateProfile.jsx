...
function CompaniesUpdateProfile() {
  ...
  function handleSubmit(e) {
    ...
    for (let i = 0; i < promises.length; i++) {
      console.log("promise arr: ", promises[i]);
    }
    //when all promises finish then will be navigated back to home page
    Promise.all(promises)
      .then(() => {
        navigate("/CompaniesUser");
      })
      .catch((error) => {
        // if there is some error with the changes, then error will be displayed
...
export default CompaniesUpdateProfile;
