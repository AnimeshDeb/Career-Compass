...
function SeekerSkills({ handleNextStep, handlePrevStep, uid }) {
  ...
  const skillsAudio =
    "https://firebasestorage.googleapis.com/v0/b/career-compass-77175.appspot.com/o/static%2Faudio%2F2024-03-15%2002-54-11.mp3?alt=media&token=4fdc488b-a513-4c48-9436-6db9b25f2068";
  ...
  async function uploadVideo(file) {
    ...
    try {
      ...
      await setDoc(docRef, { skills: downloadURL }, { merge: true });
  ...
  const handleEditorChange = (e) => {
    ...
    setSeekerTxtSkills(cleanHtml);
    if (activeArea !== "text") setActiveArea("text");
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (seekerTxtSkills.trim()) {
      try {
        await setDoc(
          docRef,
          { skills: seekerTxtSkills.trim() },
          { merge: true }
        );
      } catch (error) {
      ...
      <div className="maybolin-talk flex flex-col md:flex-row items-center justify-center m-4 mx-auto max-w-4xl">
        ...
        <div className="flex-1 bg-blue-100 px-6 py-4 mt-4 shadow-lg relative text-left mx-5 rounded-tr-lg rounded-bl-lg rounded-br-lg ">
          ...
          <div className="flex justify-end mt-0">
            <Audio_Btn audioSrc={skillsAudio} />
          </div>
        </div>
      </div>
      ...
      <div className="mt-0 mb-0 max-w-7xl mx-auto pl-4 pr-4 space-y-6 bg-white rounded-lg">
        <form onSubmit={handleSubmit}>
          <div className="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 md:space-x-4 h-auto md:min-h-120 lg:min-h-150">
            ...
            >
              <EditorTxt
                seekerTxtIntro={seekerTxtSkills}
          ...
          <div className="flex justify-between pt-2 pb-2">
            ...
            <button
              ...
              className={`px-6 py-2 text-lg rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 flex justify-center items-center ${
                isUploading
                  ...
                  : uploadComplete || seekerTxtSkills.trim()
            ...
            >
              {isUploading ? (
                ...
                <>
                  {uploadComplete || seekerTxtSkills.trim() ? "Next" : "Skip"}
                </>
              )}
            </button>
          </div>
